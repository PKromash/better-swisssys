...
class THDBF extends DBF{
    ...
    #addRecord(tournament){
        ...
        record.push(String(tournament.startDate.getFullYear() + String(tournament.startDate.getMonth() + 1).padStart(2, '0') + String(tournament.startDate.getDate()).padStart(2, '0')))
        record.push(String(tournament.endDate.getFullYear() + String(tournament.endDate.getMonth() + 1).padStart(2, '0') + String(tournament.endDate.getDate()).padStart(2, '0')))
        ...
        record.push(tournament.tournamentDirectors.map(td => td.USCF_id).join(','))
...
class TSDBF extends DBF{
    ...
    #addRecord(sections, isTest){
        ...
        for(let i = 0; i < sections.length; i++){
            ...
            //B: under 10 minutes
            let timeRating
            let result = sections[i].timeControl.match(pattern)
            ...
            if(totalTime > 65){
                ...
                throw new RangeError("Time Control is too low")
            }
            let record = []
            record.push(undefined)
            record.push(i+1)
            ...
            record.push(String(sections[i].beginningDate.getFullYear() + String(sections[i].beginningDate.getMonth() + 1).padStart(2, '0') + String(sections[i].beginningDate.getDate()).padStart(2, '0')))
            record.push(String(sections[i].endDate.getFullYear() + String(sections[i].endDate.getMonth() + 1).padStart(2, '0') + String(sections[i].endDate.getDate()).padStart(2, '0')))
            ...
            record.push(sections[i].grandPrixStatus === "N" ? undefined : sections[i].grandPrixPoints)
            record.push("N")
            this.records.push(record)
        }
    }
...
class TDDBF extends DBF{
    ...
    #addRecord(sections){
        ...
        for(let player of players){
            ...
            record.push(player.rating)
            let iter = 0
            for(let i = 0; i < sections[section-1].rounds.length; i++){
                if(player.byes.length !== 0 && player.byes.some(b => b.round === i+1)){
                    let res = ""
